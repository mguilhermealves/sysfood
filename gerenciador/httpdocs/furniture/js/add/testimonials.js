data_testimonials_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_testimonials_json.template +=
	'  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_NAME#>#NAME#</div>';
data_testimonials_json.template +=
	'  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_UNITS#>#UNITS#</div>';
data_testimonials_json.template +=
...
data_testimonials_json.template += "</div>";

data_testimonials_json.footer = '<div class="row_js table_data_footer">';
data_testimonials_json.footer += ' <div class="cell cell_last table_data_footer">';
data_testimonials_json.footer += "     Linha por Página ";
data_testimonials_json.footer += '     <select id="per_page">';
data_testimonials_json.footer += '         <option value="20" selected>20</option>';
data_testimonials_json.footer += '         <option value="50">50</option>';
data_testimonials_json.footer += '         <option value="100">100</option>';
data_testimonials_json.footer += "     </select>";
data_testimonials_json.footer += " </div>";
data_testimonials_json.footer +=
...
$.ajax({
	type: "GET",
	url: data_testimonials_json.url,
	data: data_testimonials_json.data,
	success: function (data) {
		$.each(data.total, function (i, o) {
			$("#" + i + "SpanUser").html(o);
		});
		data_testimonials_json.out_data = data;
		fn_common.render_data(
			data_testimonials_json.template + '<div class="row_data"></div>',
			{
				"#CLASS#": " row_js_header",
				"#ADD#": "",
				"#NAME#":
					'<button style="width: 100%; background-color: #ffffff; color: #707070; text-align: left; font-weight: bold; padding: 0px;" id="btn_ordenation_name" type="button">Nome da Cliente<i class="fa fa-border-none"></i></button>',
				"#ADD_NAME#": "",
				"#UNITS#":
					'<button style="width: 100%; background-color: #ffffff; color: #707070; text-align: left; font-weight: bold; padding: 0px;" id="btn_ordenation_level" type="button">Unidade<i class="fa fa-border-none"></i></button>',
				"#ADD_UNITS#": "",
				"#ACAO#": "AÇÃO",
				"#ADD_ACAO#": "",
			},
		...
		$.each(data.row, function (i, o) {
			fn_common.render_data(
				data_testimonials_json.template,
				{
					...
					"#ADD#": ' data-name="' + o.client_name + '"',
					"#ADD_NAME#": "",
					"#ADD_UNITS#": ' title="#UNITS#"',
					"#NAME#": o.client_name,
					"#ADD_ACAO#": "",
					"#UNITS#": o.units_attach.map(function (value) {
						return value.trade_name;
					}),
					"#ACAO#":
						'<a href="/depoimento/' +
						o.idx +
						'" class="btn button info round"><i class="fontello-edit"></i> Editar</a><a href="/depoimento/' +
						o.idx +
...
var fn_common = {
	...
	paginate: function (sr) {
		...
		if (paginate < data_testimonials_json.out_data.total.total) {
			last_paginate =
				data_testimonials_json.out_data.total.total % per_page == 0
					? data_testimonials_json.out_data.total.total / per_page
					: Math.floor(data_testimonials_json.out_data.total.total / per_page) + 1;
		...
		paginate_text += " de " + data_testimonials_json.out_data.total.total;
