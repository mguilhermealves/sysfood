data_banners_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_banners_json.template += ' <div class="cell" style="overflow:hidden;"#ADD_NAME#>#NAME#</div>';
data_banners_json.template += ' <div class="cell" style="overflow:hidden;"#ADD_TARGET#>#TARGET#</div>';
data_banners_json.template += ' <div class="cell" style="overflow:hidden;"#ADD_STATUS#>#STATUS#</div>';
data_banners_json.template += ' <div class="cell cell_last" style="min-width: 240px !important;display: flex;justify-content: center;align-items: center;"#ADD_ACAO#>#ACAO#</div>';
data_banners_json.template += "</div>";

data_banners_json.footer = '<div class="row_js table_data_footer">';
data_banners_json.footer += ' <div class="cell cell_last table_data_footer">';
data_banners_json.footer += "     Linha por Página ";
data_banners_json.footer += '     <select id="per_page">';
data_banners_json.footer += '         <option value="20" selected>20</option>';
data_banners_json.footer += '         <option value="50">50</option>';
data_banners_json.footer += '         <option value="100">100</option>';
data_banners_json.footer += "     </select>";
data_banners_json.footer += " </div>";
data_banners_json.footer +=
...
$.ajax({
	type: "GET",
	url: data_banners_json.url,
	data: data_banners_json.data,
	success: function (data) {
		console.log(data);
		$.each(data.total, function (i, o) {
			$("#" + i + "SpanUser").html(o);
		});
		data_banners_json.out_data = data;
		fn_common.render_data(
			data_banners_json.template + '<div class="row_data"></div>',
			{
				"#CLASS#": " row_js_header",
				"#ADD#": "",
				"#NAME#":
					'<button style="width: 100%; background-color: #ffffff; color: #707070; text-align: left; font-weight: bold; padding: 0px;" id="btn_ordenation_name" type="button">Nome do Banner <i class="fa fa-border-none"></i></button>',
				"#TARGET#":
					'<button style="width: 100%; background-color: #ffffff; color: #707070; text-align: left; font-weight: bold; padding: 0px;" id="btn_ordenation_perfil" type="button">Linkavel <i class="fa fa-border-none"></i></button>',
				"#STATUS#":
		...
		$.each(data.row, function (i, o) {
			fn_common.render_data(
				data_banners_json.template,
				{
					...
					"#ADD_ACAO#": "",
					"#NAME#": o.name,
					"#TARGET#": o.target == "yes" ? "Sim" : "Não",
					"#STATUS#": o.active == "yes" ? "Sim" : "Não",
					"#ACAO#":
						'<a href="/banner/' + o.idx + '" class="btn button info round"><i class="fontello-edit"></i> Editar</a><a href="/banner/' + o.idx + '" class="btn button alert round" id="btn_remove_' + o.idx + '"><i class="fontello-trash"></i> Excluir </a>',
				},
...
var fn_common = {
	...
	paginate: function (sr) {
		...
		if (paginate < data_banners_json.out_data.total.total) {
			last_paginate =
				data_banners_json.out_data.total.total % per_page == 0
					? data_banners_json.out_data.total.total / per_page
					: Math.floor(data_banners_json.out_data.total.total / per_page) +
					  1;
		...
		paginate_text += " de " + data_banners_json.out_data.total.total;
