data_categories_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_categories_json.template +=
...
data_categories_json.template += "</div>";

data_categories_json.footer = '<div class="row_js table_data_footer">';
data_categories_json.footer += ' <div class="cell cell_last table_data_footer">';
data_categories_json.footer += "     Linha por Página ";
data_categories_json.footer += '     <select id="per_page">';
data_categories_json.footer += '         <option value="20" selected>20</option>';
data_categories_json.footer += '         <option value="50">50</option>';
data_categories_json.footer += '         <option value="100">100</option>';
data_categories_json.footer += "     </select>";
data_categories_json.footer += " </div>";
data_categories_json.footer +=
...
$.ajax({
	type: "GET",
	url: data_categories_json.url,
	data: data_categories_json.data,
	success: function (data) {
		$.each(data.total, function (i, o) {
			$("#" + i + "SpanUser").html(o);
		});
		data_categories_json.out_data = data;
		fn_common.render_data(
			data_categories_json.template + '<div class="row_data"></div>',
			{
				"#CLASS#": " row_js_header",
				"#ADD#": "",
				"#NAME#":
			...
			fn_common.render_data(
				data_categories_json.template,
				{
					...
					"#ACAO#":
						'<a href="/categoria/' +
						o.idx +
						'" class="btn button info round"><i class="fontello-edit"></i> Editar</a><a href="/categoria/' +
						o.idx +
...
var fn_common = {
	...
	paginate: function (sr) {
		...
		if (paginate < data_categories_json.out_data.total.total) {
			last_paginate =
				data_categories_json.out_data.total.total % per_page == 0
					? data_categories_json.out_data.total.total / per_page
					: Math.floor(data_categories_json.out_data.total.total / per_page) + 1;
		...
		paginate_text += " de " + data_categories_json.out_data.total.total;
