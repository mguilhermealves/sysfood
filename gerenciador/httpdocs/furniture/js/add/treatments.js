data_treatments_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_treatments_json.template +=
	'  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_NAME#>#NAME#</div>';
data_treatments_json.template +=
	'  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_TYPES#>#TYPES#</div>';
data_treatments_json.template +=
...
data_treatments_json.template += "</div>";

data_treatments_json.footer = '<div class="row_js table_data_footer">';
data_treatments_json.footer += ' <div class="cell cell_last table_data_footer">';
data_treatments_json.footer += "     Linha por Página ";
data_treatments_json.footer += '     <select id="per_page">';
data_treatments_json.footer += '         <option value="20" selected>20</option>';
data_treatments_json.footer += '         <option value="50">50</option>';
data_treatments_json.footer += '         <option value="100">100</option>';
data_treatments_json.footer += "     </select>";
data_treatments_json.footer += " </div>";
data_treatments_json.footer +=
...
$.ajax({
	type: "GET",
	url: data_treatments_json.url,
	data: data_treatments_json.data,
	success: function (data) {
		$.each(data.total, function (i, o) {
			$("#" + i + "SpanUser").html(o);
		});
		data_treatments_json.out_data = data;
		fn_common.render_data(
			data_treatments_json.template + '<div class="row_data"></div>',
			{
				"#CLASS#": " row_js_header",
				"#ADD#": "",
				"#NAME#":
				...
				"#ADD_NAME#": "",
				"#TYPES#":
					'<button style="width: 100%; background-color: #ffffff; color: #707070; text-align: left; font-weight: bold; padding: 0px;" id="btn_ordenation_level" type="button">Tipo<i class="fa fa-border-none"></i></button>',
				"#ADD_TYPES#": "",
				"#ACAO#": "AÇÃO",
				"#ADD_ACAO#": "",
			},
		...
		$.each(data.row, function (i, o) {
			fn_common.render_data(
				data_treatments_json.template,
				{
					...
					"#ADD_NAME#": "",
					"#ADD_TYPES#": ' title="#TYPES#"',
					"#NAME#": o.name,
					"#ADD_ACAO#": "",
					"#TYPES#": o.typestreatments_attach.map(function (value) {
						return value.name;
					}),
					"#ACAO#":
						'<a href="/tratamento/' +
						o.idx +
						'" class="btn button info round"><i class="fontello-edit"></i> Editar</a><a href="/tratamento/' +
						o.idx +
...
var fn_common = {
	...
	paginate: function (sr) {
		...
		if (paginate < data_treatments_json.out_data.total.total) {
			last_paginate =
				data_treatments_json.out_data.total.total % per_page == 0
					? data_treatments_json.out_data.total.total / per_page
					: Math.floor(data_treatments_json.out_data.total.total / per_page) + 1;
		...
		paginate_text += " de " + data_treatments_json.out_data.total.total;
