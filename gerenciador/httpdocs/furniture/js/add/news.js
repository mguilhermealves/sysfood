data_news_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_news_json.template +=
...
data_news_json.template += "</div>";

data_news_json.footer = '<div class="row_js table_data_footer">';
data_news_json.footer += ' <div class="cell cell_last table_data_footer">';
data_news_json.footer += "     Linha por Página ";
data_news_json.footer += '     <select id="per_page">';
data_news_json.footer += '         <option value="20" selected>20</option>';
data_news_json.footer += '         <option value="50">50</option>';
data_news_json.footer += '         <option value="100">100</option>';
data_news_json.footer += "     </select>";
data_news_json.footer += " </div>";
data_news_json.footer +=
...
$.ajax({
	type: "GET",
	url: data_news_json.url,
	data: data_news_json.data,
	success: function (data) {
		$.each(data.total, function (i, o) {
			$("#" + i + "SpanUser").html(o);
		});
		data_news_json.out_data = data;
		fn_common.render_data(
			data_news_json.template + '<div class="row_data"></div>',
			{
				"#CLASS#": " row_js_header",
				"#ADD#": "",
				"#NAME#":
			...
			fn_common.render_data(
				data_news_json.template,
				{
					...
					"#ACAO#":
						'<a href="/noticia/' +
						o.idx +
						'" class="btn button info round"><i class="fontello-edit"></i> Editar</a><a href="/noticia/' +
						o.idx +
...
var fn_common = {
	...
	paginate: function (sr) {
		...
		if (paginate < data_news_json.out_data.total.total) {
			last_paginate =
				data_news_json.out_data.total.total % per_page == 0
					? data_news_json.out_data.total.total / per_page
					: Math.floor(data_news_json.out_data.total.total / per_page) + 1;
		...
		paginate_text += " de " + data_news_json.out_data.total.total;
