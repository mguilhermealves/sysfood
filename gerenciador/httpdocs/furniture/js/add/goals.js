data_goals_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_goals_json.template += '  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_NAME#>#NAME#</div>';
data_goals_json.template += '  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_LEVEL#>#LEVEL#</div>';
data_goals_json.template += '  <div class="cell cell_last" style="height: 45px;text-align:center !important;min-width: 70px !important;overflow:hidden;"#ADD_ACAO#>#ACAO#</div>';
data_goals_json.template += '</div>';

data_goals_json.footer = '<div class="row_js table_data_footer">';
data_goals_json.footer += ' <div class="cell cell_last table_data_footer">';
data_goals_json.footer += '     Linha por Página ';
data_goals_json.footer += '     <select id="per_page">';
data_goals_json.footer += '         <option value="20" selected>20</option>';
data_goals_json.footer += '         <option value="50">50</option>';
data_goals_json.footer += '         <option value="100">100</option>';
data_goals_json.footer += '     </select>';
data_goals_json.footer += ' </div>';
data_goals_json.footer += ' <div class="cell cell_last table_data_footer" style="justify-content: space-around;" id="paginate_control"></div>';
data_goals_json.footer += ' <div class="cell cell_last table_data_footer" id="paginate_display">#DATA_TOTAL#</div>';
data_goals_json.footer += '</div>';

$.ajax({
    type: "GET",
    url: data_goals_json.url
    , data: data_goals_json.data
    ,success: function( data ){
        $.each( data.total , function(i,o){
            $( "#" + i + "SpanUser").html( o )
        })
        data_goals_json.out_data = data ;
        fn_common.render_data( 
            data_goals_json.template + '<div class="row_data"></div>'
            , {
                "#CLASS#" : " row_js_header"
                , "#ADD#": ""
                ...
                , "#ADD_NAME#" : ''
                , "#LEVEL#" : '<button style="width: 100%; background-color: #ffffff; color: #707070; text-align: left; font-weight: bold; padding: 0px;" id="btn_ordenation_level" type="button">Perfil<i class="fa fa-border-none"></i></button>'
                , "#ADD_LEVEL#" : ''
                , "#ACAO#" : 'AÇÃO'
                , "#ADD_ACAO#" : ''
            }
        ...
        $.each( data.row , function(i,o){
            fn_common.render_data( 
                ...
                , {
                    ...
                    , "#LEVEL#" :  o.profiles_attach.map( function( value ) { return value.name; })
                    , "#ADD_ACAO#" : ''
                    , "#ACAO#" :  '<a href="/meta/' + o.idx + '" class="btn button info round"><i class="fontello-edit"></i> Editar</a>'
                }
...
var fn_common = {
    ...
    paginate: function( sr ){
        ...
        if( paginate < data_goals_json.out_data.total.total ){
            last_paginate = data_goals_json.out_data.total.total % per_page == 0 ? data_goals_json.out_data.total.total / per_page : ( Math.floor( data_goals_json.out_data.total.total / per_page ) + 1 ) ;
        ...
        paginate_text += ' de ' + data_goals_json.out_data.total.total;
