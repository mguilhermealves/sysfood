data_tables_json.template = '<div class="row_js#CLASS#"#ADD#>';
data_tables_json.template +=
	'  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_CHAIRS#>#NAME#</div>';
data_tables_json.template +=
	'  <div class="cell" style="height: 45px;min-width: 70px !important;overflow:hidden;"#ADD_CHAIRS#>#CHAIRS#</div>';
data_tables_json.template +=
...
data_tables_json.template += "</div>";

data_tables_json.footer = '<div class="row_js table_data_footer">';
data_tables_json.footer += ' <div class="cell cell_last table_data_footer">';
data_tables_json.footer += "     Linha por Página ";
data_tables_json.footer += '     <select id="per_page">';
data_tables_json.footer += '         <option value="20" selected>20</option>';
data_tables_json.footer += '         <option value="50">50</option>';
data_tables_json.footer += '         <option value="100">100</option>';
data_tables_json.footer += "     </select>";
data_tables_json.footer += " </div>";
data_tables_json.footer +=
...
$.ajax({
	type: "GET",
	url: data_tables_json.url,
	data: data_tables_json.data,
	success: function (data) {
		$.each(data.total, function (i, o) {
			$("#" + i + "SpanUser").html(o);
		});
		data_tables_json.out_data = data;
		fn_common.render_data(
			data_tables_json.template + '<div class="row_data"></div>',
			{
				"#CLASS#": " row_js_header",
				"#ADD#": "",
				"#NAME#":
				...
				"#ADD_CHAIRS#": "",
				"#CHAIRS#":
					'<button style="width: 100%; background-color: #ffffff; color: #707070; text-align: left; font-weight: bold; padding: 0px;" id="btn_ordenation_level" type="button">Qtd de Lugares<i class="fa fa-border-none"></i></button>',
				"#ADD_LEVEL#": "",
				"#ACAO#": "AÇÃO",
				"#ADD_ACAO#": "",
			},
			...
			fn_common.render_data(
				data_tables_json.template,
				{
					...
					"#ACAO#":
						'<a href="/mesa/' +
						o.idx +
						'" class="btn button info round"><i class="fontello-edit"></i> Editar</a><a href="/mesa/' +
						o.idx +
...
var fn_common = {
	...
	paginate: function (sr) {
		...
		if (paginate < data_tables_json.out_data.total.total) {
			last_paginate =
				data_tables_json.out_data.total.total % per_page == 0
					? data_tables_json.out_data.total.total / per_page
					: Math.floor(data_tables_json.out_data.total.total / per_page) + 1;
		...
		paginate_text += " de " + data_tables_json.out_data.total.total;
